 ...
 */
const RAMPS: Readonly<Record<FxnkTheme, FxnkRamp>> = {
  dark: {
...
export function fxnkRamp(theme: FxnkTheme): FxnkRamp {
  return RAMPS[theme];
}
...
export class FxnkThemeMonitor {
  ...
  private readonly inputHandler = (sequence: string): boolean => {
    ...
    if (notification) {
      // FX_THEME fixes the palette for the lifetime of the process. Still own
      // the protocol byte so OpenTUI cannot start a second theme query path.
