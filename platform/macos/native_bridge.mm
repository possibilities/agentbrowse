...
- (instancetype)initWithFrame:(NSRect)frame callbacks:(KLNativeCallbacks)callbacks {
...
- (instancetype)initWithCallbacks:(KLNativeCallbacks)callbacks title:(NSString *)title {
  self = [super init];
  if (!self) return nil;
  _callbacks = callbacks;
  _windowTitle = title.length ? title : @"Kernel Live View";
...
- (void)emitState:(KLNativeState)state {
  if (_callbacks.on_state) _callbacks.on_state(_callbacks.context, state);
}
...
- (void)createWindow {
  ...
  self.inputView = [[KLInputView alloc] initWithFrame:frame callbacks:self.callbacks];
...
- (void)receiveNextMessage {
  ...
  [task receiveMessageWithCompletionHandler:^(NSURLSessionWebSocketMessage *message, NSError *error) {
    ...
    if (!strongSelf || strongSelf.closing || task != strongSelf.webSocket) return;
    if (error) {
...
- (void)setRemoteAnswer:(BOOL)answer sdp:(NSString *)sdp {
  ...
  [peer setRemoteDescription:description completionHandler:^(NSError *error) {
    KLNativeSession *strongSelf = weakSelf;
    if (!strongSelf || strongSelf.closing || peer != strongSelf.peer) return;
    if (error) {
      [strongSelf reportError:@"set remote description failed"];
      return;
    }
    if (answer) return;
    LKRTCMediaConstraints *constraints = [[LKRTCMediaConstraints alloc] initWithMandatoryConstraints:nil optionalConstraints:nil];
    [peer answerForConstraints:constraints completionHandler:^(LKRTCSessionDescription *local, NSError *answerError) {
      if (strongSelf.closing || peer != strongSelf.peer) return;
      if (answerError || !local) {
        [strongSelf reportError:@"create answer failed"];
        return;
      }
      [peer setLocalDescription:local completionHandler:^(NSError *localError) {
        if (strongSelf.closing || peer != strongSelf.peer) return;
        if (localError) {
          [strongSelf reportError:@"set local description failed"];
          return;
        }
        if (strongSelf.callbacks.on_local_description) {
          KLBytes(local.sdp, ^(const uint8_t *bytes, size_t len) {
            strongSelf.callbacks.on_local_description(strongSelf.callbacks.context, true, bytes, len);
          });
        }
      }];
    }];
...
- (void)addCandidateJSON:(NSString *)candidateJSON {
  ...
  [peer addIceCandidate:ice completionHandler:^(NSError *error) {
    if (error && !self.closing && peer == self.peer) {
      [self reportError:@"add ICE candidate failed"];
    }
  }];
}
...
- (void)resetTransport {
  ...
  if (self.videoTrack) {
    ...
    [self.videoTrack removeRenderer:self.inputView.videoView];
  }
  self.videoTrack = nil;
...
- (void)applicationWillTerminate:(NSNotification *)notification {
  (void)notification;
  if (_callbacks.on_close) _callbacks.on_close(_callbacks.context);
}
...
- (void)peerConnection:(LKRTCPeerConnection *)peerConnection didGenerateIceCandidate:(LKRTCIceCandidate *)candidate {
  if (self.closing || peerConnection != self.peer) return;
  if (!_callbacks.on_local_candidate) return;
...
- (void)peerConnection:(LKRTCPeerConnection *)peerConnection didAddReceiver:(LKRTCRtpReceiver *)receiver streams:(NSArray<LKRTCMediaStream *> *)mediaStreams {
  ...
  [self.videoTrack addRenderer:self.inputView.videoView];
}
...
extern "C" KLNativeSessionHandle *kl_native_create(KLNativeCallbacks callbacks,
                                                     ...
                                                     size_t title_len) {
  @autoreleasepool {
    KLNativeSession *session = [[KLNativeSession alloc]
...
extern "C" bool kl_native_send_data(KLNativeSessionHandle *handle,
  ...
  if (!session.dataChannel || session.dataChannel.readyState != LKRTCDataChannelStateOpen) return false;
...
extern "C" void kl_native_start_heartbeat(KLNativeSessionHandle *handle,
  ...
  KLNativeSession *session = KLSession(handle);
  dispatch_async(dispatch_get_main_queue(), ^{
    [session.heartbeat invalidate];
    session.heartbeat = [NSTimer scheduledTimerWithTimeInterval:(interval_ms / 1000.0)
                                                        repeats:YES
                                                          block:^(NSTimer *timer) {
      (void)timer;
      static const uint8_t message[] = "{\"event\":\"client/heartbeat\"}";
      [session sendWebSocketBytes:message length:sizeof(message) - 1];
    }];
  });
}
...
extern "C" void kl_native_set_status(KLNativeSessionHandle *handle,
                                       const uint8_t *bytes, size_t len) {
  KLNativeSession *session = KLSession(handle);
  NSString *status = KLString(bytes, len);
  dispatch_async(dispatch_get_main_queue(), ^{
    session.inputView.statusLabel.stringValue = status;
  });
}

extern "C" int kl_native_run(KLNativeSessionHandle *handle) {
  @autoreleasepool {
    KLNativeSession *session = KLSession(handle);
