...
**Current delta is under fixture review:** original UUID fencing and official idle-timeout driver exit replace the earlier pending cleanup guard. No Studio rehearsal or deployment acceptance yet.
...
The action file is a JSON array. Supported actions are `focus {selector}`, `scrollintoview {selector}`, `click {selector}`, `fill {selector,value}`, `press {key}`, `wait {ms}`, `expectText {selector,value}`, `expectValue {selector,value}` and `expectOpen {selector,value:"true"|"false"}`. Selectors and keys use installed agent-browser syntax. At most 100 actions are accepted; each wait is at most ten seconds. Expectations use bounded read-only reconciliation, never repeated input. Arbitrary eval, navigation, app restarts, uploads, downloads and shell commands are not action types.
...
Normal order is: stop/reap encoder, read guest size/hash, copy bounded chunks to a private local file, verify hash, fully decode, verify monotonically increasing source timestamps and the stable lead-in, revoke the bridge, then await official driver idle exit and exact provider-detach acknowledgement, revalidate the saved VM identity and release the saved lease with its expected target/profile/backend/original instance UUID, confirming lease absence. Existing outputs are not overwritten.

The opt-in capture provider uses the existing plugin protocol. It permits one launch only and retains ownership on driver detach; the helper alone performs release. The official driver uses a 30-second idle timeout. Cleanup waits at most 45 seconds for the captured PID to be absent and the exact lease detach receipt; it sends no process signals or name-only close. PID reuse conservatively leaves pending. Original UUID mismatch/missing state, ambiguous launch, missing acknowledgement or failure to exit preserves pending state. After recording begins, a failed or cancelled run revokes access and stops the guest encoder but retains the exact lease/source as `pending` for explicit recovery. The private manifest records the lease, target, instance ID and an allocated guest source path before dispatching recorder start. If allocation itself fails, no recorder start is dispatched. This is intentional retained recovery state, not a healthy recording or an automatic retry. There is not yet a public recovery subcommand: inspect/copy/hash/decode that exact owned source through the authenticated exec API before releasing the exact lease. The test investigation exercised this recovery manually and retained its receipt. A production-facing recovery command and a crash-after-copy acceptance test remain follow-up work before broad distribution.
...
## Same-target preparation and native supervisor handoff

Use `--prepare-wait 600 --coordination-wait 5` with a script path that the producer will author. Preparation accepts60..900 seconds; 600 gives10minutes before native recording starts. The helper publishes `preparation-evidence.json` with exact session/lease/VM/page identity plus fresh read-only snapshot and screenshot. It alone drives the browser, refreshing evidence every10seconds to retain the same daemon during authoring. Preparation performs no tuning or other app writes.

After authoring from observed controls, the supervisor binds its effects plan, validates its native baseline and starts its recorder, then atomically publishes `prepare-ready.json` matching preparationId, identitySha256 and SHA256 of the exact script file bytes. The helper accepts those bytes once, rechecks the same target and records it. Invalid/late ready never authorizes input. Guest exec has a finite300+prepareSeconds lifetime; recording retains its separate5..120-second bound. The Studio supervisor must fit its own60-second native ceiling, including readiness and stop margin.

When coordination is enabled, each action requires an exact intent hash permit before genuine dispatch and an exact completion hash acknowledgement before the next action. The supervisor owns the full native-state check; host fixture acknowledgements do not prove it. All barriers are bounded by coordination wait and remaining recording time. `supervisor-abort.json` irreversibly stops dispatch; `helper-failure.json` is published immediately on latched failure. `browser-capture-stopped.json` is published before copy/decode/idle cleanup so native stop does not wait for helper exit.

Exact filenames, envelopes, hash definition, zero-based action indexing and start/stop semantics are in [preparation protocol](../../review/preparation-protocol.md). No second driver owner, no timeout-as-consent, no replay of uncertain actions. The disposable coordinated fixture deliberately waits35seconds during authoring (beyond driver idle timeout) and checks that its next intent is withheld until mock acknowledgement; it has no phone/native acceptance claim.

Preparation evidence also includes a fixed read-only `controls` observation: current numeric values, raw min/max/step, disabled/readOnly state, label, unique-ID selector, bounding rectangle and viewport intersection, plus select options. It is capped at256 controls/64 options with explicit truncation flags. Text/password values are omitted. Use this alongside snapshot/screenshot to bind the real observed numeric control; missing attributes are null, not guessed defaults. This does not add a general evaluation API or perform input/DOM mutation.

Guarded selector-only actions `focus` and `scrollintoview` use those exact official driver commands. They perform no click or value assignment. The helper records before/after current value, checked/selected state, raw numeric attributes and rectangles; it rejects unexpected changes. After a bounded rectangle-settle check it revalidates page/VM/display geometry and places the real native pointer using that fresh rectangle. Focus must be observed on the exact element. The native pointer moves once then reads back X coordinates, avoiding the identical-position hang of xdotool --sync. Each action uses the same intent/permit/completion/ack budget; native supervisor effects remain unchanged. Keyboard changes are separate guarded `press` actions, with every actual intermediate value bound explicitly. PageDown/PageUp must not be assumed inverse.
